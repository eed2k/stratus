...
class Config:
    ...
    def __init__(self):
        ...
        self.PULSE_MIRROR_PIN = 19   # Strike pulse out to the Campbell logger
        # Width of that pulse. The CR300 counts switch closures up to 150 Hz,
        # so a few milliseconds is already detectable; 50 ms sits well above any
        # input debounce while still permitting 10 strikes per second, far more
        # than this unit ever forwards.
        self.PULSE_MIRROR_WIDTH_MS = 50
...
class LightningDetector:
    ...
    _CONFIG_VALIDATORS = {
        ...
        "PULSE_MIRROR_WIDTH_MS": (int, 5, 500),
    ...
    def _irq_callback(self, channel):
        ...
        # The strike pulse is deliberately NOT raised here. It used to be, and
        # was cleared again in _process_interrupt's finally, which made the pulse
        # width equal to the whole processing time: a 2 ms sensor settle, SPI
        # reads, the bit-banged serial record, and a webhook POST that can take
        # up to ALERT_WEBHOOK_TIMEOUT seconds. Two faults followed from that.
        # A second strike arriving inside the window re-asserted an already-high
        # line, so no new edge was produced and a burst was counted once. And
        # every interrupt raised it, NOISE_HIGH included, so the logger counted
        # events that were not strikes. It is now emitted as a fixed-width pulse
        # for confirmed lightning only, from _process_interrupt.
    ...
    def _emit_strike_pulse(self):
        """Emit one fixed-width pulse per confirmed strike on the mirror pin.

        Blocking on purpose. The width is tens of milliseconds against a strike
        path that already spends longer than that on the serial record and the
        webhook, so the cost is immaterial. Driving both edges inline is what
        guarantees every strike gets its own complete high-low cycle instead of
        merging into its neighbour, which is exactly what the previous
        raise-on-interrupt, clear-after-processing scheme got wrong.
        """
        if not self.config.PULSE_MIRROR_ENABLED:
            return
        try:
            GPIO.output(self.config.PULSE_MIRROR_PIN, GPIO.HIGH)
            time.sleep(self.config.PULSE_MIRROR_WIDTH_MS / 1000.0)
            GPIO.output(self.config.PULSE_MIRROR_PIN, GPIO.LOW)
        except RuntimeError:
            # Mirrors the tolerance elsewhere in this class: a GPIO fault must
            # never cost us the strike record itself.
            pass
    ...
    def _process_interrupt(self):
        ...
        try:
            ...
            if int_type == INT_LIGHTNING:
                ...
                # Hardware strike output to the Campbell logger, one pulse per
                # confirmed strike. Emitted even when the interference guard
                # mutes the outbound alerts, so this path stays a witness that
                # depends on neither the network nor the serial line. It can
                # therefore exceed the logger's serial StrikeCount during an
                # interference window, and that difference is the diagnostic.
                self._emit_strike_pulse()
            ...
            # Safety net only. _emit_strike_pulse owns both edges of the pulse
            # and returns the line low itself; this guarantees the line is not
            # left asserted if it raised part way through.
            if self.config.PULSE_MIRROR_ENABLED:
                try:
                    GPIO.output(self.config.PULSE_MIRROR_PIN, GPIO.LOW)
                except RuntimeError:
    ...
    def start(self):
        ...
        self.logger.info(
            ...
            (
                ...
                f"{self.config.PULSE_MIRROR_WIDTH_MS} ms pulse)"
