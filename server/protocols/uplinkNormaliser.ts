...
/**
 * Uplink normaliser for the HTTP POST ingest endpoint.
 *
...
export interface NormalisedUplink {
  format: UplinkFormat;
  /**
 ...
 * Never throws: an unrecognised body comes back as format "unknown".
 */
export function normaliseUplink(body: any): NormalisedUplink {
  const format = detectUplinkFormat(body);
  const result: NormalisedUplink = {
    format,
    deviceIds: [],
    timestamp: null,
    decoded: {},
...
export function describeUplinkFormat(format: UplinkFormat): string {
  switch (format) {
    ...
    default: return "Unrecognised payload";
  }
}
