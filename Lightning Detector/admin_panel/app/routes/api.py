...
from ..models import AlertEvent, MessageLog, HeartbeatSample
...
def ingest_heartbeat(payload: HeartbeatPayload, _=Depends(verify_token),
    ...
    # Record a time-series sample for the 24h CPU trend chart.
    try:
        ...
        # Prune samples older than 48h to keep the table small.
        cutoff = now_sast() - timedelta(hours=48)
...
def beacon_state(_=Depends(verify_token), db: Session = Depends(get_db)):
    ...
    units = get_units(db, threshold_s)
    ...
    latest = (db.query(AlertEvent)
                .filter(AlertEvent.timestamp >= bound,
