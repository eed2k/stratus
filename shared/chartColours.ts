...
/**
 * Canonical chart colours for Stratus.
 *
 * Before this existed the same quantity was drawn in different colours on
 ...
 * that, and it is the single place to change a colour.
 *
 * House rules baked in here:
 *   - ETo is always green, and battery voltage is always green.
 *   - No purple or violet, and no pale washed-out blues. Every colour is
...
export type ChartColourKey = keyof typeof CHART_COLOURS;

/** Look up a colour, falling back to the brand navy for anything unmapped. */
export function chartColour(key: string): string {
  return (CHART_COLOURS as Record<string, string>)[key] ?? CHART_COLOURS.brand;
}
