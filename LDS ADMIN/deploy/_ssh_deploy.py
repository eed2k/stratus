...
WARNING - the defaults below point at a host that no longer answers.

139.84.238.225 (gwld1-admin.dynv6.net) was the panel's own dedicated VPS. It
does not respond on SSH or HTTPS any more. The panel is now served from
adminpanel.stratusweather.co.za, which resolves to 139.84.242.126 - the SAME
box that serves stratusweather.co.za.

That matters because deploy/deploy_on_vps.sh installs Caddy and OVERWRITES
/etc/caddy/Caddyfile with a single-domain config. The shared box fronts Stratus
with Traefik on ports 80/443, so running that script there would fight Traefik
for the ports and drop the Stratus vhost. Read the warning at the top of
deploy_on_vps.sh before pointing this at the shared host.

Override the target with the DEPLOY_HOST / DEPLOY_DOMAIN / DEPLOY_REMOTE
...
HOST = os.environ.get("DEPLOY_HOST", "139.84.238.225")
USER = os.environ.get("DEPLOY_USER", "root")
REMOTE = os.environ.get("DEPLOY_REMOTE", "/opt/lightning-panel")
DOMAIN = os.environ.get("DEPLOY_DOMAIN", "gwld1-admin.dynv6.net")
...
def _password():
    """Root password for the VPS, from the environment or the secret file.

    There is deliberately no hardcoded fallback. This folder is committed, so a
    literal password here would end up in git history. Supply it as
    DEPLOY_PASSWORD in the environment, or put it on one line in
    deploy/.deploy_secret (which is gitignored).
    """
